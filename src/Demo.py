...
class PositionUpdater():
	...
	self.hand_pickandplace = PP.PickAndPlace('left', True)
        self.step = 0.0596
    ...
    def advance(self, name, move):
        if len(move) != 4:
            ...
            if '_b_' in name:
                ...
                # TODO Pick up chess
                self.hand_pickandplace.pick(prev_pos.x, prev_pos.y, prev_pos.z)
		
                self.hand_planner.move_hand_interface(prev_pos.x + delta_x, prev_pos.y + delta_y, prev_pos.z + self.offset)
                # TODO Drop chess
                self.hand_pickandplace.place(prev_pos.x + delta_x, prev_pos.y + delta_y, prev_pos.z)
